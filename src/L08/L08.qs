...
namespace MITRE.QSD.L08 {
    ...
    import MITRE.QSD.L03.E02_ReverseRegister;
    ...
    operation E01_QFT (register : Qubit[]) : Unit is Adj + Ctl {
        ...
        for i in 0 .. Length(register) - 1 {
            ...
            // Apply controlled phase gates for all subsequent qubits
    ...
    operation E02_EstimateFrequency (
        ...
        mutable result = MeasureInteger(register);
        Message($"Result: {result}");
        // If the result is greater than half the size of the register, mirror it
        if result >= 2 ^ Length(register) / 2 {
            Message($"Mirroring result: {result}");
            result = 2 ^ Length(register) - result;
        }


        mutable freq = IntAsDouble(result) * sampleRate / IntAsDouble(2 ^ Length(register));
        Message("size:"+DoubleAsStringWithPrecision(IntAsDouble(2^Length(register)), 3));
        Message("rate:" + DoubleAsStringWithPrecision(sampleRate, 3));
        Message(DoubleAsStringWithPrecision(freq, 5));
